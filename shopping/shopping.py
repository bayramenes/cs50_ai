...
def load_data(filename):
    ...
    # evidense list that we will store
    evidences = []
    labels = []
    ...
    with open(filename,"r") as file:
        ...
        for row in csv_reader:
            ...
            # add month
            evidence.append(MONTHS.index(row[10]))
...
def train_model(evidence, labels):
    ...
    """
    # we would usually want to split the data into training and testing sets
    # but that is already done in the load_data function so no need to do so
...
def evaluate(labels, predictions):
    ...
    for i in range(len(labels)):
        if labels[i] == 1:
            ...
            if labels[i] == predictions[i]:
                true_negative += 1
    sensitivity = true_positive / positive_labels
    specificity = true_negative / negative_labels
